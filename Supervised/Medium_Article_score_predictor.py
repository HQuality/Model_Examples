...
#%%
def preprocess_c(path_to_inp_json_file, path_to_out_txt_file):
    output_list = []
    with open(path_to_inp_json_file, encoding='utf-8') as inp_file,             open(path_to_out_txt_file, 'w', encoding='utf-8') as out_file:
            for line in tqdm_notebook(inp_file):
                ...
                lz = ltz.lemmatize(content_no_html_tags)
                out_file.write(lz + '\n')


#%%
def preprocess_a(path_to_inp_json_file, path_to_out_txt_file):
    df = pd.DataFrame()
    with open(path_to_inp_json_file, encoding='utf-8') as inp_file,             open(path_to_out_txt_file, 'w', encoding='utf-8') as out_file:
            for line in tqdm_notebook(inp_file):
                json_data = read_json_line(line)
                author = json_data["meta_tags"]["article:author"].replace('\n', ' ').replace('\r', ' ')
                out_file.write(author + '\n')


#%%
def preprocess_t(path_to_inp_json_file, path_to_out_txt_file):
    output_list = []
    with open(path_to_inp_json_file, encoding='utf-8') as inp_file,             open(path_to_out_txt_file, 'w', encoding='utf-8') as out_file:
            for line in tqdm_notebook(inp_file):
                json_data = read_json_line(line)
                content = json_data["meta_tags"]["title"].replace('\n', ' ').replace('\r', ' ')
                content_no_html_tags = strip_tags(content)
                lz= ltz.lemmatize(content_no_html_tags)
                out_file.write(lz + '\n')
...
#%%
def preprocess_p(path_to_inp_json_file, path_to_out_txt_file):
    output_list = []
    with open(path_to_inp_json_file, encoding='utf-8') as inp_file,             open(path_to_out_txt_file, 'w', encoding='utf-8') as out_file:
            for line in tqdm_notebook(inp_file):
                json_data= read_json_line(line)
                published = json_data["meta_tags"]["article:published_time"].replace('\n', ' ').replace('\r', ' ')
                out_file.write(published + '\n')


#%%
def preprocess_d(path_to_inp_json_file, path_to_out_txt_file):
    output_list = []
    with open(path_to_inp_json_file, encoding='utf-8') as inp_file,             open(path_to_out_txt_file, 'w', encoding='utf-8') as out_file:
            for line in tqdm_notebook(inp_file):
                json_data= read_json_line(line)
                datal = json_data["meta_tags"]["twitter:data1"].replace('\n', ' ').replace('\r', ' ')
                out_file.write(datal + '\n')
...
#%%
#json.dumps(first_json, indent=4, sort_keys=False, ensure_ascii=False)
...
#%%
preprocess_d(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'train.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'train_raw_datal.txt'))
preprocess_d(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'test.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'test_raw_datal.txt'))


#%%
preprocess_c(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'train.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'train_raw_content.txt'))


#%%
preprocess_c(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'test.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'test_raw_content.txt'))


#%%
preprocess_a(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'train.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'train_raw_author.txt'))


#%%
preprocess_a(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'test.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'test_raw_author.txt'))


#%%
preprocess_t(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'train.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'train_raw_title.txt'))


#%%
preprocess_t(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'test.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'test_raw_title.txt'))


#%%
preprocess_p(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'train.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'train_raw_published.txt'))


#%%
preprocess_p(path_to_inp_json_file=os.path.join(PATH_TO_DATA, 'test.json'),
           path_to_out_txt_file=os.path.join(PATH_TO_DATA, 'test_raw_published.txt'))


#%%
ltz = nltk.stem.wordnet.WordNetLemmatizer()
...
def preprocessText(data):
    ...
    for line in tqdm_notebook(data):
        preprocessed.append([ltz.lemmatize(word) for word in line])
    return preprocessed


#%%
tf1 = TfidfVectorizer(ngram_range=(1,2), max_features=250000, max_df=0.5)
tf2 = TfidfVectorizer(ngram_range=(1,3), max_features=250000, max_df=0.5)

cn = CountVectorizer(ngram_range=(1,3), max_features=250000, min_df=1, max_df=0.3, analyzer='char')
...
#%%
with open(os.path.join(PATH_TO_DATA, 'train_raw_content.txt'), encoding='utf-8') as input_train_file:
    train_c = tf1.fit_transform(input_train_file)
...
#%%
with open(os.path.join(PATH_TO_DATA, 'test_raw_content.txt'), encoding='utf-8') as input_train_file:
    test_c = tf1.transform(input_train_file)
...
#%%
with open(os.path.join(PATH_TO_DATA, 'train_raw_title.txt'), encoding='utf-8') as input_train_file:
    train_title = tf2.fit_transform(input_train_file)
...
#%%
with open(os.path.join(PATH_TO_DATA, 'test_raw_title.txt'), encoding='utf-8') as input_train_file:
    test_title = tf2.transform(input_train_file)


#%%
with open(os.path.join(PATH_TO_DATA, 'train_raw_title.txt'), encoding='utf-8') as input_train_file:
    train_char = cn.fit_transform(input_train_file)


#%%
with open(os.path.join(PATH_TO_DATA, 'test_raw_title.txt'), encoding='utf-8') as input_train_file:
    test_char = cn.transform(input_train_file)


#%%
full_char = sp.sparse.vstack([train_char, test_char]).tocsr()


#%%
with open(os.path.join(PATH_TO_DATA, 'train_raw_published.txt'), encoding='utf-8') as input_train_file:
    train_published = pd.read_csv(input_train_file, sep=" ", header=None)
    train_published.columns = ["date"]


#%%
with open(os.path.join(PATH_TO_DATA, 'test_raw_published.txt'), encoding='utf-8') as input_train_file:
    test_published = pd.read_csv(input_train_file, sep=" ", header=None)
    test_published.columns = ["date"]
...
#%%
with open(os.path.join(PATH_TO_DATA, 'train_raw_datal.txt'), encoding='utf-8') as input_train_file:
    train_datal = pd.read_csv(input_train_file, sep=" ", header=None)
    train_datal.columns = ["timeread",'id1','id2']


#%%
with open(os.path.join(PATH_TO_DATA, 'test_raw_datal.txt'), encoding='utf-8') as input_train_file:
    test_datal = pd.read_csv(input_train_file, sep=" ", header=None)
    test_datal.columns = ["timeread",'id1','id2']


#%%
full_datal = pd.concat([train_datal, test_datal])


#%%
full_datal['1_5'] = full_datal.timeread.apply(lambda x:1 if x in range(1,6) else 0)
full_datal['6_9'] = full_datal.timeread.apply(lambda x:1 if x in range(6,10) else 0)
full_datal['10_15'] = full_datal.timeread.apply(lambda x:1 if x in range(11,16) else 0)
full_datal['16_20'] = full_datal.timeread.apply(lambda x:1 if x in range(16,21) else 0)
full_datal['21_30'] = full_datal.timeread.apply(lambda x:1 if x in range(21,31) else 0)
full_datal['31'] = full_datal.timeread.apply(lambda x:1 if x in range(31,1000) else 0)


#%%
full_datal = full_datal.drop(['timeread','id1','id2'], axis=1)


#%%
train_published['date'] =  pd.to_datetime(train_published['date'])
test_published['date'] =  pd.to_datetime(test_published['date'])
...
#%%
full_published = pd.concat([train_published, test_published])


#%%
full_published['Hour'] = full_published['date'].apply(lambda x:x.strftime('%H')).astype('int64')
full_published['Day_of_week'] = full_published['date'].dt.strftime('%w').astype('int64')
full_published['Month'] = full_published.date.dt.strftime('%m').astype('int64')


#%%
full_published['Morning'] = full_published.Hour.apply(lambda x:1 if x in range(6,11) else 0)
full_published['Day'] = full_published.Hour.apply(lambda x:1 if x in range(12,18) else 0)
full_published['Eve'] = full_published.Hour.apply(lambda x:1 if x in range(19,22) else 0)
full_published['Night'] = full_published.Hour.apply(lambda x:1 if x in range(23,5) else 0)


#%%
full_published['is_summer'] = full_published['Month'].apply(lambda x: 1 if x in (6, 7, 8) else 0)
full_published['is_weekend'] = full_published['date'].apply(lambda x: 1 if x.date().weekday() in (5, 6) else 0)


#%%
hour = enc.fit_transform(full_published[['Hour']].as_matrix())
dayweek = enc.fit_transform(full_published[['Day_of_week']].as_matrix())
month = enc.fit_transform(full_published[['Month']].as_matrix())


#%%
#hour_full = sp.sparse.vstack([hour, hour_test]).tocsr()


#%%
#dayweek_full = sp.sparse.vstack([dayweek, dayweek_test]).tocsr()


#%%
full_published = full_published.drop(['date', 'Hour', 'Day_of_week', 'Month'], axis=1)


#%%
with open(os.path.join(PATH_TO_DATA, 'train_raw_author.txt'), encoding='utf-8') as input_train_file:
    train_author = pd.read_csv(input_train_file, sep=" ", header=None)
    train_author.columns = ["author"]


#%%
with open(os.path.join(PATH_TO_DATA, 'test_raw_author.txt'), encoding='utf-8') as input_train_file:
    test_author = pd.read_csv(input_train_file, sep=" ", header=None)
    test_author.columns = ["author"]


#%%
full_author = pd.concat([train_author,test_author])
...
#%%
full_author['id'] = pd.RangeIndex(start=0, stop=96958, step=1)
full_author['occur'] = train_author.groupby('author')['author'].transform(len)
...
#%%
full_author['1pub'] = full_author.occur.apply(lambda x: 1 if x in range (1,2) else 0)
full_author['2pub'] = full_author.occur.apply(lambda x: 1 if x in range (2,3) else 0)
full_author['3pub'] = full_author.occur.apply(lambda x: 1 if x in range (3,4) else 0)
full_author['4-5pub'] = full_author.occur.apply(lambda x: 1 if x in range (4,6) else 0)
full_author['6-7pub'] = full_author.occur.apply(lambda x: 1 if x in range (6,8) else 0)
full_author['8-10pub'] = full_author.occur.apply(lambda x: 1 if x in range (8,11) else 0)
full_author['11-15pub'] = full_author.occur.apply(lambda x: 1 if x in range (11,16) else 0)
full_author['16-22pub'] = full_author.occur.apply(lambda x: 1 if x in range (16,23) else 0)
full_author['23-30pub'] = full_author.occur.apply(lambda x: 1 if x in range (23,31) else 0)
full_author['31-45pub'] = full_author.occur.apply(lambda x: 1 if x in range (31,46) else 0)
full_author['46-61pub'] = full_author.occur.apply(lambda x: 1 if x in range (46,62) else 0)
full_author['62-100pub'] = full_author.occur.apply(lambda x: 1 if x in range (62,101) else 0)
full_author['101pub'] = full_author.occur.apply(lambda x: 1 if x in range(102, 1000) else 0)


#%%
tra = pd.get_dummies(full_author['author'])


#%%
full_author = full_author.drop(['author','id', 'occur'], axis=1)


#%%
full_c = sp.sparse.vstack([train_c, test_c]).tocsr()


#%%
full_t = sp.sparse.vstack([train_title, test_title]).tocsr()


#%%
xtrain = csr_matrix(hstack([train_c, train_title, train_published, train_author])).tocsr()


#%%
xtest = csr_matrix(hstack([test_c, test_title, test_published, test_author])).tocsr()


#%%
del ([train_published, test_published, train_author, test_author, train_datal, test_datal, train_c, test_c, train_title, test_title])
...
#%%
xfull = csr_matrix(hstack([full_c, full_t, full_datal, full_author, tra, full_published])).tocsr()
...
#%%
Xfull_train = xfull[:idx_split, :]
Xfull_test = xfull[idx_split:, :]
...
X_train_part = xtrain[:train_part_size, :]
y_train_part = y_train[:train_part_size]
X_valid =  xtrain[train_part_size:, :]
y_valid = y_train[train_part_size:]


#%%
ridge=Ridge(alpha=2, random_state=7)


#%%
ridge.fit(Xfull_train, y_train)


#%%
ridge_pred = ridge.predict(Xfull_test)
...
#%%
huber.fit(Xfull_train, y_train)


#%%
huber_pred = huber.predict(Xfull_test)
...
#%%
grid = GridSearchCV(ridge, param_grid=params, cv=5, scoring='neg_mean_absolute_error', n_jobs=-1)
...
#%%
score = mean_absolute_error(y_valid, grid_pred)

#%% [markdown]
# test_published['is_summer'].values.reshape(-1, 1) ---> 1.1286187757499286
# 
...
#%%
sgdparams = {'penalty':['l2'],
            'max_iter':[100],
            'random_state':[17]}


#%%
sgd_grid = GridSearchCV(sgd, param_grid=sgdparams, cv=5, scoring='neg_mean_absolute_error', n_jobs=-1, verbose=1)


#%%
sgd_grid.fit(X_train_part, y_train_part)
...
#%%
sgd_grid_pred = sgd_grid.predict(X_valid)
sgdscore=mean_absolute_error(y_valid, sgd_grid_pred)
sgdscore
...
#%%
get_ipython().run_cell_magic('time', '', 'ridge.fit(Xfull_train, y_train);')
...
#%% [markdown]
# (1.0879057993956158, 1.9680518635536828) a1
# 
# (1.0831442005797409, 1.953952785031345) a2

#%%
crack_mae = mean_absolute_error(y_valid, np.zeros_like(ridge_pred))
crack_mae, np.expm1(crack_mae)
...
#%%
xfull = csr_matrix(hstack([full_c, full_t, hour_full, dayweek_full, full_published['is_summer'].values.reshape(-1, 1), full_published['is_weekend'].values.reshape(-1, 1)])).tocsr()


#%%
ridge.fit(xfull, y_train)
...
#%%
write_submission_file(ridge_pred, '555_medium_submission.csv')


#%%
write_submission_file(huber_pred, '13huber_medium_submission.csv')
...
write_submission_file(np.zeros_like(ridge_test_pred), 
                      'zmedium_all_zeros_submission.csv')
...
#%%
np.log1p(ridge_test_pred[1:24])


#%%
yt = 1.35*ridge_test_pred.size
...
#%%
ridge_modif = ridge_pred + 0.680
#остроумная, но не умная "калибровка" предиктов

#%%
write_submission_file(ridge_modif, 
                      'assignment6_medium_submission_680.csv')
...
# Идеи для улучшения:
# 
# Работа с признаками это, само собой, главный ключ хорошей модели. Дата публикации, размер статьи, автор, и тп
# Залезть в html и вытащить что-то еще оттуда по ключам - святое дело
# Улучшить схему валидацию (так, чтобы cv и lb коррелировали)
# Правильно настроенный TF-IDF с n_grams, + word2vec/GLoVe
# Стемминг, лемматизация
# Потом тюнинг гиперпараметров, конечно
# SGD и online learning алгоритмы (VW) будут намного, намного шустрее
# Нейронки (LSTM?)
